...
Machine: 20 cores, RTX 4070 (8 GB), Windows 11, native CUBERT SDK 3.6.0 (build `987fdcf`), published `cuvis` / `cuvis-il` 3.6.0.0 wheels.
...
The host column is flat within its own noise (2.70-3.78 cubes/s, no trend), so read it as "about 3 cubes/s whatever the thread count".
The per-cell ratios inherit that wobble: the 15.0x at four threads is a low host cell, not a better GPU one.
...
- **The GPU is 5.4x faster before any threading**, 16.79 against 3.12 cubes/s. That is the number a default `num_workers: 0`, `read_threads: 0` config gets back.
- **Threading is a GPU-only lever.** On the host the whole 1-to-8 sweep stays inside 2.70-3.78 cubes/s with no trend, while the GPU goes 16.79 to 49.25, a 2.9x. The two features are therefore not independent - `read_threads` without `sdk_cuda` is dead weight, and the combined gap at eight threads is 13.5x.
- **Host mode is not a cheaper mode, it is a fallback.** It costs 5-14x and buys no memory back that matters at these thread counts.
...
- **Setup is device-independent**, 12.83-13.74 s in every cell. The `ProcessingContext` LUT build dominates process start-up either way, so switching to the host does not even save the wait.

Do not over-read:

- **Do not compare these cubes/s against `benchmarks/threaded_reading/report.md`.** That run reached 52.6 cubes/s at eight threads where this one reaches 49.3, on the same machine and session. The two runs differ in GPU thermal state and in what ran immediately before, and only within-report rows are controlled against each other. Both reports' *shapes* agree; their absolute cells do not transfer.
